"""
Công cụ Tạo mã QR Sản phẩm & Xuất file PDF In Ấn (Smart Intralogistics QR Generator)
Hỗ trợ cả chế độ nhập tương tác (Interactive CLI) và tham số dòng lệnh (Command Line Arguments).

Cách dùng:
  1. Chạy trực tiếp nhập bàn phím:
...
def generate_qr_pdf(
    ...
    # Tạo ảnh QR tạm thời
    temp_qr_path = f"temp_qr_{product_id}.png"
    ...
    main_card_data = [
        ...
        [qr_img_element],
        [info_table],
        [Paragraph(f"Nội dung mã QR: <i>{qr_content_str}</i>", ParagraphStyle("Mini", fontSize=8, leading=10, textColor=colors.gray, alignment=1))],
    ]
    ...
    print(f"[OK] Da tao thanh cong file PDF nhan QR: {os.path.abspath(output_pdf_path)}")

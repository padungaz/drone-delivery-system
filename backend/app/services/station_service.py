...
class StationService:
    ...
    async def execute_unload_product(self, target_slot: str, product_id: str, session: AsyncSession) -> bool:
        ...
        try:
            ...
            # 10. Assign product to Storage Slot in Inventory
            await inv_mgr.update_slot(target_slot, StorageSlotStatus.OCCUPIED, product_id=product_id)
            await self._broadcast_status("10. ASSIGN_SLOT", f"Đã cất sản phẩm {product_id} vào ô {target_slot} thành công.", status="RUNNING")
            ...
            await self._broadcast_status("12. TAKEOFF_COMPLETE", f"Drone đã rời bãi N1 (DRONE_DETECT=0). Hoàn thành chu trình nhập ô {target_slot}!", status="COMPLETED")
            return True

...
async def main():
    ...
    async with async_session() as session:
        ...
        await plc_mgr.move_z_to_level(1)  # Hàng A (DB15.DBW8=1)
        st_za = plc_mgr.get_status()
        logger.info("✓ PLC move_z_to_level(1/Row A) executed. Z Axis: %s, plc_z_in_position: %s, level: %d", st_za.z_axis, st_za.plc_z_in_position, st_za.current_z_level)
        await plc_mgr.move_z_to_level(0)  # Home (DB15.DBW8=0)
        st_zhome = plc_mgr.get_status()
        logger.info("✓ PLC move_z_to_level(0/HOME) executed. Z Axis: %s, plc_z_in_position: %s, level: %d", st_zhome.z_axis, st_zhome.plc_z_in_position, st_zhome.current_z_level)

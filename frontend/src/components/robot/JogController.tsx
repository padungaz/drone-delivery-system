...
export function JogController({ onJogCommand, systemMode = "AUTO" }: Props) {
    ...
    <div className="hmi-card jog-controller">
      <div className="card-header flex-between">
        ...
        {systemMode === "MANUAL" ? (
          ...
          <span className="mode-indicator-tag auto-locked">AUTO LOCKED</span>
        )}
      </div>

      <div className="card-body jog-layout">
        {systemMode === "AUTO" ? (
          <div className="auto-mode-status-container" style={{ padding: "0.8rem" }}>
            <div className="auto-status-header">
              <span className="pulse-dot-green"></span>
              <strong className="text-green font-mono">JOG TỰ ĐỘNG KHÓA</strong>
            </div>
            <p className="auto-mode-desc" style={{ fontSize: "0.72rem", margin: "0.4rem 0" }}>
              Để đảm bảo an toàn va chạm, các phím điều khiển nhích từng trục (JOG) bị khóa khi hệ thống đang ở chế độ <strong>AUTO</strong>.
            </p>
            <div className="auto-specs-mini">
              <div className="flex-between mini-row">
                <span>Chế độ di chuyển:</span>
                <strong className="text-cyan font-mono">TỰ ĐỘNG THEO QUỸ ĐẠO</strong>
              </div>
            </div>
          </div>
        ) : (
